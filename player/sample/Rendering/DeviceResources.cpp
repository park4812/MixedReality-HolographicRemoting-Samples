...
void
DX::DeviceResources::ReleaseSwapChainPanel()
{
    LOG("ReleaseSwapChainPanel");
...
{
    LOG("CreateDeviceIndependentResources");
    ...
    winrt::check_hresult(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof(ID2D1Factory3), &options,
                                           reinterpret_cast<void**>(m_d2dFactory.put())));

    // Initialize the DirectWrite Factory.
    winrt::check_hresult(
        DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory3), reinterpret_cast<::IUnknown**>(m_dwriteFactory.put())));
...
DX::DeviceResources::CreateDeviceResources()
{
    ...
    if (FAILED(hr))
    {
        ...
        winrt::check_hresult(D3D11CreateDevice(nullptr,
                                               ...
                                               0, creationFlags, featureLevels, ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, device.put(),
                                               &m_d3dFeatureLevel, context.put()));
    }

    // Store pointers to the Direct3D 11.3 API device and immediate context.
    m_d3dDevice = device.try_as<ID3D11Device3>();
    m_d3dContext = context.try_as<ID3D11DeviceContext3>();

    // Create the Direct2D device object and a corresponding context.
    winrt::com_ptr<IDXGIDevice3> dxgiDevice = m_d3dDevice.try_as<IDXGIDevice3>();
...
DX::DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    else
    {
        ...
        winrt::check_hresult(dxgiFactory->CreateSwapChainForComposition(m_d3dDevice.get(), &swapChainDesc, nullptr, swapChain.put()));
        ...
        m_swapChainPanel.Dispatcher().RunAsync(CoreDispatcherPriority::High, DispatchedHandler([=]() {
                                                   winrt::com_ptr<ISwapChainPanelNative> panelNative =
                                                       m_swapChainPanel.as<ISwapChainPanelNative>();
    ...
    winrt::com_ptr<IDXGISwapChain2> spSwapChain2 = m_swapChain.try_as<IDXGISwapChain2>();
    ...
    winrt::check_hresult(m_d2dContext->CreateBitmapFromDxgiSurface(dxgiBackBuffer.get(), &bitmapProperties, m_d2dTargetBitmap.put()));
...
void
DX::DeviceResources::UpdateRenderTargetSize()
{
    LOG("UpdateRenderTargetSize");
...
void
DX::DeviceResources::SetSwapChainPanel(SwapChainPanel const& panel)
{
    LOG("SetSwapChainPanel");
...
void
DX::DeviceResources::SetLogicalSize(Windows::Foundation::Size logicalSize)
{
    LOG("SetLogicalSize");
...
void
DX::DeviceResources::SetDpi(float dpi)
{
    LOG("SetDpi");
...
void
DX::DeviceResources::SetCurrentOrientation(DisplayOrientations currentOrientation)
{
    LOG("SetCurrentOrientation");
...
void
DX::DeviceResources::SetCompositionScale(float compositionScaleX, float compositionScaleY)
{
    LOG("SetCompositionScale");
...
void
DX::DeviceResources::ValidateDevice()
{
    LOG("ValidateDevice");
...
void
DX::DeviceResources::RegisterDeviceNotify(DX::IDeviceNotify* deviceNotify)
{
    LOG("RegisterDeviceNotify");
...
DXGI_MODE_ROTATION
DX::DeviceResources::ComputeDisplayRotation()
{
    LOG("ComputeDisplayRotation");
