...
// 애플리케이션의 상태를 프레임마다 한 번씩 업데이트합니다.
HolographicFrame BasicHologramMain::Update()
{
	// Before doing the timer update, there is some work to do per-frame
	// to maintain holographic rendering. First, we will get information
	// about the current frame.
    // 타이머 업데이트를 수행하기 전에, 홀로그래픽 렌더링을 유지하기 위해 프레임마다 수행해야 할 일이 있습니다. 먼저, 현재 프레임에 대한 정보를 얻을 것입니다.

	// The HolographicFrame has information that the app needs in order
	// to update and render the current frame. The app begins each new
	// frame by calling CreateNextFrame.
    // HolographicFrame에는 앱이 현재 프레임을 업데이트하고 렌더링하는 데 필요한 정보가 있습니다. 앱은 CreateNextFrame을 호출함으로써 각 새 프레임을 시작합니다.

	HolographicFrame holographicFrame = m_holographicSpace.CreateNextFrame();

	// Get a prediction of where holographic cameras will be when this frame
	// is presented.
    // 이 프레임이 제시될 때 홀로그래픽 카메라가 어디에 있을지 예측을 얻습니다.
...
bool BasicHologramMain::Render(HolographicFrame const& holographicFrame)
{
        ...
        [this, holographicFrame](std::map<UINT32, std::unique_ptr<DX::CameraResources>>& cameraResourceMap)
    {
        ...
        for (HolographicCameraPose const& cameraPose : prediction.CameraPoses())
        {
            ...
            // 위치 추적이 활성화되어 있을 때만 월드-락(content that is fixed in the real world) 컨텐츠를 렌더링하세요.
            if (cameraActive)
            {
